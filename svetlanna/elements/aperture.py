...
class Aperture(MulElement):
    ...
    def transmission_function_axes(self) -> tuple[str, ...]:
        """
        Returns the axes used for the transmission function calculation.

            Args:
                None

            Returns:
                tuple[str, ...]: A tuple of strings representing the axis labels
                                 used in calculating the transmission function.
        """
        return self._calc_axes

    def get_transmission_function(self) -> torch.Tensor:
        """
        Returns the transmission function (mask).

          Args:
            None

          Returns:
            torch.Tensor: The mask representing the transmission function.
        """
        return self.mask

    def to_specs(self) -> Iterable[ParameterSpecs]:
        """
        Returns parameter specifications for the mask.

            Args:
                None

            Returns:
                Iterable[ParameterSpecs]: An iterable of ParameterSpecs objects,
                                          containing representations of the mask.
        """
        return [
            ParameterSpecs(
                "mask",
...
class RectangularAperture(MulElement):
    ...
    def to_specs(self) -> Iterable[ParameterSpecs]:
        """
        Returns parameter specifications for height and width.
            ...
            Returns:
                Iterable[ParameterSpecs]: An iterable of ParameterSpecs objects,
                                          containing specs for 'height' and 'width'.
        """
        return [
...
class RoundAperture(MulElement):
    ...
    def to_specs(self) -> Iterable[ParameterSpecs]:
        """
        Returns parameter specifications for the radius.

            Args:
                None

            Returns:
                Iterable[ParameterSpecs]: A list of ParameterSpecs, currently containing only
                                          the specification for the 'radius' parameter.
        """
