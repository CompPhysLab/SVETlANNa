...
def test_detector_types():
    ...
    detector = Detector(
        SimulationParameters(
            {
                "x": torch.linspace(-1e-2 / 2, 1e-2 / 2, 5),
                "y": torch.linspace(-1e-2 / 2, 1e-2 / 2, 5),
                "wavelength": 1e-6,
            }
        )
    )
...
def test_detector_intensity(x_size, y_size, x_nodes, y_nodes, wavelength):
    ...
    detector = Detector(
        SimulationParameters(
            {
                "x": torch.linspace(-x_size / 2, x_size / 2, x_nodes),
                "y": torch.linspace(-y_size / 2, y_size / 2, y_nodes),
...
def test_detector_segmentation_strips(num_classes, detector_x, expected_mask):
    ...
    processor = DetectorProcessorClf(
        ...
        simulation_parameters=SimulationParameters(
            {
                "x": torch.linspace(-1e-2 / 2, 1e-2 / 2, detector_x),
...
def test_detector_weight_segments(num_classes, segmented_detector, expected_weights):
    ...
    processor = DetectorProcessorClf(
        ...
        simulation_parameters=SimulationParameters(
            {
                "x": torch.linspace(-1e-2 / 2, 1e-2 / 2, all_detector_size[1]),
                "y": torch.linspace(-1e-2 / 2, 1e-2 / 2, all_detector_size[0]),
                "wavelength": 500e-6,
            }
        ),
